...
def __repr__(self):
      return f'<Show {self.date} {self.venue_id} {self.artist_id} >'
...
#  Venues try 
#  ----------------------------------------------------------------
  # q=db.session.query(Venue).join(Venue.show_ven).all()
    # s=db.session.query(Venue).group_by(Venue.city)
    # print("this is the answer:",s)
       # data1.sort(key=operator.itemgetter('city'))
    # print(data1)
    # # group the department in a list
    # list1=[]
    # for key , item in itertools.groupby(data1, operator.itemgetter('city')):\
...
def venues():
  ...
  #       num_shows should be aggregated based on number of upcoming shows per venue.
    ...
    # count_event=Show.query.filter(Show.date >= datetime.today()).all()
...
# sourec https://stackoverflow.com/questions/19699756/flask-sqlalchemy-greater-than-or-equal-to
...
def search_venues():
  ...
  response1={
    "count":result.count(),
    "data":result
    #  "num_upcoming_shows": 0, not required
  }
  # print(result)
  response={
    "count": 1,
...
def search_artists():
  ...
  search_term= request.form.get('search_term', '')
  response1=Artist.query.filter(Artist.name.ilike(f'%search_term%'))
...
def show_artist(artist_id):
  ...
  data_all=Artist.query.order_by(Artist.id).all()
...
# art=Artist(name = request.form['name'],
#     city = request.form['city'],
#     state = request.form['state'],
#     phone = request.form['phone'],
#     genres = request.form.getlist('genres'),
...
def shows():
  ...
  shows_info= db.session.query(\
     Venue.id, Venue.name , Artist.id , Artist.name , Artist.image_link, Show.date).join(\
      Show, Show.artist_id==Artist.id).join(\
        Venue, Show.artist_id == Venue.id).order_by(Artist.id).subquery()
  #Disply the data of three table 
...
def create_shows():
  ...
  return render_template('forms/new_show.html', form=form, ids=id)
